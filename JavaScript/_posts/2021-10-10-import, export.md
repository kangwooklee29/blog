...
\- 원래 표준 자바스크립트는 이와 같은 이점을 추구하는 모듈화 관련 문법을 갖고 있지 않았다. 그러나 '자바스크립트가 브라우저 밖에서도 쓸 수 있는 언어가 돼야 한다'라고 주장한 이들이 이를 위한 문법 표준인 CommonJS를 제안하면서 여기에 모듈화 관련 문법을 포함시켰고, 이후 표준 자바스크립트 문법에 모듈화 관련 문법을 포함시켜야 한다는 목소리도 강해져 결국 2015년 발표된 ES6에서 모듈화 관련 문법이 포함되었다. 아래는 그 내용이다.
