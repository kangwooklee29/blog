...
```HTMl
fetch("confing.json").then(response => console.log(response.status));
```
...
\-  Promise 객체를 쓰는 경우 보통 그 뒤에 .then() 구문을 줄줄이 쓰게 되는데, 이것이 길어지면 가독성이 나빠질 수 있다. await 예약어는 이러한 Promise 객체 사용을 개선할 수 있는 예약어로, Promise 객체를 반환하는 함수 앞에 await 예약어를 쓰면 (1)마치 동기 함수를 호출하는 것과 같이 그 함수가 Promise 객체를 반환할 때까지 기다리며 (2)리턴된 Promise 객체를 Promise 객체의 결과값으로 변환한다. 따라서 await 예약어를 사용하면 Promise 객체를 반환하는 함수를 마치 동기 함수 쓰듯 쓸 수 있다. 예를 들어 3번에서 썼던 다음 코드를 await 예약어를 이용해 마치 동기식 코드처럼 보이게끔 쓸 수 있다.
```javascript
fetch("config.json")
    .then(response => response.json())
    .then(console.log)
