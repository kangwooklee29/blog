...
\- 단, **내부에서 await 예약어가 사용되는 함수는 항상 function 예약어 앞에 반드시 async 예약어가 붙어야 한다(위 코드의 'async function printConfig() 구문을 보라).** async 예약어가 붙어있지 않은 함수 내에 await 예약어가 사용되면 에러가 발생한다.
