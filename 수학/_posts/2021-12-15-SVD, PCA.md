...
\- 서로 직교하는 관계인 벡터끼리는 서로 선형독립이기도 하다. (단, 선형독립 관계인 벡터는 반드시 서로 직교하는 것은 아니다.)


#### 2) 고유값 분해

\- \\(n \times n\\) 행렬이 고유값을 \\(n\\)개 이상 갖는다면 그 행렬을 그때의 고유벡터들로 만든 행렬과 각 성분이 고유값인 대각행렬의 곱셈 꼴로 표현할 수 있으며, 이를 고유값 분해(eigenvalue decomposition)이라 한다.
...
이때 만약 \\(P^{-1}\\)이 존재한다면 \\(A = P \Lambda P^{-1}\\)라는 식을 쓸 수 있다.
