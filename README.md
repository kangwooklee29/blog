...
1. post 쓸 때 YAML 헤더에 title에 콜론을 쓸 일이 종종 있는데, 콜론을 넣으면 YAML 헤더의 속성 중 하나로 지킬이 인식하는 문제 해결 가능한지 궁금.

2. 지킬 자체 syntax highlighter를 못 쓰겠는데, 코드블럭의 언어를 C++로 쓰면 이를 인식 못하는 경우가 있어서. 이거 극복 가능한 문제인지 궁금.

3. 그래서 티스토리에서 흔히 쓰는 highlightjs를 설치했는데, 이건 종종 코드블럭 내 코드가 짤리는 문제가 있어서.. 이것도 극복 가능한 문제인지..
