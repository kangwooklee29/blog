...
- 새 속성명/테이블명에 띄어쓰기나 SQL 쿼리의 예약어가 있다면 ""로 묶어써야 한다.
...
(b) NULLIF(field1, 0), COALESCE(field1, field2, ..., 0)

\- 인자의 첫 번째 값부터 NULL이 아니면 그냥 그 값을 그대로 리턴하고, NULL이면 두 번째 값을 리턴하는데 그게 NULL이면 다시 세 번째 값을 리턴하고, ... 하는 작업을 수행한다. NULLIF는 두 개의 인자만 받지만 COALESCE는 그 이상도 가능하다. (정해진 개수는 없다.)
