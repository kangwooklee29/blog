...
\- table1의 field1값과 table2의 field2값이 일치하는 튜플이 일대 다로 여럿이라면 하나를 복제한 후 거기에 일치하는 모든 튜플을 **모두 결합하여** 가져온다. (이런 속성이 있기 때문에, **실수로 어느 한 테이블에 중복 튜플이 삽입되게 되면 JOIN 연산을 통해 오차가 기하급수적으로 커질 위험이 있다.** 따라서 DB에서 중복 튜플이 있는지를 점검하는 일은 매우 중요한 문제가 된다.)
